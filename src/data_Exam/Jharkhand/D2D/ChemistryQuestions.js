...
/* ================= COMPONENT ================= */
export default function ChemistryQuestions() {
  ...
  function openYearQuestions(yearObj) {
    ...
    if (yearObj.key === "ALL") {
      Object.values(chemistryQuestionsByYear).forEach(arr => {
        qs = qs.concat(arr);
      });
    } else {
      ...
      {viewMode === "years" && (
        <div className="pyq-list">
          {years.map((y, i) => {
            ...
            if (y.key === "ALL") {
              Object.values(chemistryQuestionsByYear).forEach(arr => total += arr.length);
            } else {
              total = chemistryQuestionsByYear[y.year]?.length || 0;
            }
