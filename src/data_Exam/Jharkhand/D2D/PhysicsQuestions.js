...
export default function PhysicsQuestions({ backToYearList }) {
  const years = [
  ...
  // Sample questions per year
  const questionsByYear = {
    "2025 Questions": [
      { id: "2025-q1", text: "Speed of light in vacuum?", options: ["3x10^8 m/s","1.5x10^8 m/s","3x10^6 m/s","1x10^8 m/s"], correctIndex: 0 },
      { id: "2025-q2", text: "Newton's Second Law?", options: ["F=ma","E=mc^2","V=IR","pV=nRT"], correctIndex: 0 },
      { id: "2025-q3", text: "Unit of electric current?", options: ["Ampere","Volt","Ohm","Watt"], correctIndex: 0 },
      { id: "2025-q4", text: "Acceleration due to gravity?", options: ["9.8 m/s²","10 m/s²","9 m/s²","8 m/s²"], correctIndex: 0 },
      { id: "2025-q5", text: "Ohm's Law?", options: ["V=IR","P=VI","F=ma","E=mc^2"], correctIndex: 0 },
      { id: "2025-q6", text: "Unit of energy?", options: ["Joule","Watt","Newton","Pascal"], correctIndex: 0 },
    ],
    "2024 Questions": [
      { id: "2024-q1", text: "Formula for work?", options: ["W=F*d","W=mv^2","W=1/2mv^2","W=Pt"], correctIndex: 0 },
      { id: "2024-q2", text: "Unit of power?", options: ["Watt","Joule","Newton","Ampere"], correctIndex: 0 },
      { id: "2024-q3", text: "Acceleration formula?", options: ["a = Δv/Δt","v=d/t","F=ma","s=ut+1/2at^2"], correctIndex: 0 },
    ],
  ...
  function openYearQuestions(yearObj) {
    const qs = questionsByYear[yearObj.year] || [];
    setYearQuestions(qs);
    setCurrentIndex(0);
    setViewMode("viewer");
  ...
  return (
    <div className="physics-box">
      <h5 className="section-title">Physics Previous Year Questions</h5>
      <div className="small text-muted mb-2">{attemptedCount} attempted</div>

      {viewMode === "years" && (
        <div className="year-list">
          {years.map((y, i) => {
            const total = questionsByYear[y.year]?.length || 0;
            const attempted = Object.keys(checked).filter(k => checked[k] && questionsByYear[y.year]?.some(q => q.id === k)).length;
            return (
              <div
                key={i}
