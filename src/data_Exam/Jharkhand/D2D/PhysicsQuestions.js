    ...
    <div className="physics-box">
      ...
      {viewMode === "years" && (
        <div className="pyq-list">
          {years.map((y, i) => {
            let total = 0;
            if (y.key === "ALL") Object.values(questionsByYear).forEach(arr => total += arr.length);
            else total = questionsByYear[y.year]?.length || 0;

            const attempted = Object.keys(checked).filter(
              k => checked[k] && (y.key === "ALL" || questionsByYear[y.year]?.some(q => q.id === k))
            ).length;
      ...
      {viewMode === "viewer" && yearQuestions.length > 0 && (
        <div className="mcq-viewer">
          <div className="exam-topbar">
            <div className="exam-left"><strong>Jharkhand D2D</strong><span>Physics</span></div>
