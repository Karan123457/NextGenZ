// src/data_Exam/Jharkhand/Polytechnic/MainPage.js
import React, { useState, useEffect, useRef } from "react";
import ChemistryQuestions, { chemistryQuestionsByYear } from "./ChemistryQuestions";
import MathematicsQuestions, { mathematicsQuestionsByYear } from "./MathematicsQuestions";
import PhysicsQuestions, { physicsQuestionsByYear } from "./PhysicsQuestions";
...
export default function PolytechnicMain() {
  ...
  // timer settings
  const QUESTION_TIME_SECONDS = 60; // Timer: 60s
  ...
  useEffect(() => {
    // Example small progress animation when switching to Analysis
  ...
  useEffect(() => {
    if (viewMode === "viewer" && yearQuestions.length > 0) {
      ...
      setIsDisabled(Boolean(qid && checked[qid])); // if already checked earlier, keep disabled
    }
  ...
  function startTimer() {
    ...
    timerRef.current = setInterval(() => {
      ...
      // if you want auto-time-up behavior:
      if (timeLeft >= QUESTION_TIME_SECONDS) {
        handleTimeUp();
      }
    }, 1000);
  }
  ...
  function goNext() {
    if (currentIndex < yearQuestions.length - 1) {
      ...
      setIsDisabled(false);
      const nextQ = yearQuestions[nextIndex];
  ...
  function goPrevious() {
    if (currentIndex > 0) {
      ...
      setIsDisabled(false);
      const prevQ = yearQuestions[prevIndex];
  ...
  // compute totals per subject (sum of all years defined in each subject module)
  const totalQuestions = (byYear) =>
    Object.values(byYear || {}).reduce(
      (s, arr) => s + (Array.isArray(arr) ? arr.length : 0),
      0
    );
  const totals = {
    phy: totalQuestions(physicsQuestionsByYear),
    chem: totalQuestions(chemistryQuestionsByYear),
    math: totalQuestions(mathematicsQuestionsByYear),
  };
  ...
  return (
    <Container className="py-5">
      ...
      <Card className="details-card">
        ...
        <Card.Body>
          <div className="details-title">{card.title || "Polytechnic PYQ"}</div>
          <div className="details-info-line">2018 – 2025 | Multiple Papers | Sample Questions</div>
          ...
          {selectedTab === "Analysis" && <Analysis progress={progress} totals={totals} />}
